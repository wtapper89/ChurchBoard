...
function moduleConfigurationMarkup(module){
  ...
  if(id==="mics")return wirelessMarkup();
  ...
  if(id==="behringer-mixer")return `${toggle("Enable Behringer mixer control","behringer.enabled")}${selectField("Console family","behringer.model",[["x32","X32 / M32"],["wing","WING / WING Compact / WING Rack"]])}<div class="module-form-columns">${field("Console IP address","behringer.host",{placeholder:"192.168.1.70"})}${field("OSC port","behringer.port",{type:"number",min:1,max:65535,fallback:10023})}</div><div class="module-inline-actions"><button class="button secondary" type="button" data-module-test="behringer">Save &amp; test console</button></div><p class="hint">X32/M32 normally uses UDP 10023. WING normally uses UDP 2223. Use a trusted, wired production network.</p>`;
...
async function openModuleSettings(module){
  ...
  if(module.id==="mics")try{modulePositionTeams=(await api("/api/integrations/planning-center/catalog")).items||[]}catch(error){modulePositionTeams=[]}
...
document.querySelector("#module-configuration").addEventListener("change",event=>{if(event.target.dataset.setting==="prodmesh_rta.mode"){collectSettings(event.currentTarget);openModuleSettings({id:"prodmesh-rta"});return}if(event.target.dataset.setting==="behringer.model"){const port=event.currentTarget.querySelector('[data-setting="behringer.port"]');if(port&&["10023","2223",""] .includes(port.value))port.value=event.target.value==="wing"?"2223":"10023"}});
document.querySelector("#module-configuration").addEventListener("click",async event=>{
  ...
  try{
    if(event.target.closest("[data-save-module-settings]")){collectSettings(root);if(form.dataset.configModule==="mics")collectWireless();await saveModuleSettings(root,status,"Module settings saved.");await openModuleSettings({id:form.dataset.configModule});status.textContent="Module settings saved.";return}
    ...
    const test=event.target.closest("[data-module-test]");if(test){collectSettings(root);if(form.dataset.configModule==="mics")collectWireless();await saveModuleSettings(root,status,"Testing…");const paths={"planning-center":"/api/integrations/planning-center/test",osm:"/api/integrations/osm/test",prodmesh:"/api/integrations/prodmesh-rta/test",behringer:"/api/integrations/behringer/test",lighting:"/api/integrations/lighting/buttons",restream:"/api/integrations/restream/test",ndi:"/api/integrations/ndi/sources"},result=await api(paths[test.dataset.moduleTest],{method:["ndi","lighting"].includes(test.dataset.moduleTest)?"GET":"POST"});if(test.dataset.moduleTest==="planning-center"){moduleSettingsData.planning_center.service_types=result.items||[];await saveModuleSettings(root,status,"Connection ready.");await openModuleSettings({id:"planning-center"})}status.textContent=result.message||`${result.count??result.items?.length??0} item${(result.count??result.items?.length)!==1?"s":""} found`;return}
