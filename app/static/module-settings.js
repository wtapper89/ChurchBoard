...
function renderCoreSettings(){
  ...
  document.querySelector("#core-settings-fields").innerHTML=`
    ...
    <fieldset><legend>Web server</legend><div class="module-form-columns">${field("Dashboard and setup port","server.port",{type:"number",min:1,max:65535,fallback:8040})}${field("Volunteer workspace port","server.producer_port",{type:"number",min:1,max:65535,fallback:80})}</div>${toggle("Use a separate volunteer workspace port","server.producer_port_enabled")}${toggle("Use HTTPS on both listeners","server.https_enabled")}<div class="module-form-columns">${field("TLS certificate file","server.ssl_certfile",{placeholder:"/path/to/fullchain.pem"})}${field("TLS private key file","server.ssl_keyfile",{placeholder:"/path/to/privkey.pem"})}</div><p class="hint">Restart ChurchBoard after changing ports or HTTPS. Mobile intercom microphone access requires trusted HTTPS.</p></fieldset>`;
}
...
function cleanSettingsPayload(){
  ...
  for(const [section,names] of Object.entries({planning_center:["secret_configured"],restream:["client_secret_configured","access_token_configured"],obs:["password_configured"],intercom:["api_key_configured","api_secret_configured","server_status","server_ports"]}))for(const name of names)delete payload[section]?.[name];
  return payload;
}
