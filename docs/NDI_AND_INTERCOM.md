...
On the production LAN, allow ChurchBoard through the operating-system firewall. The hosted engine prefers signaling `7880`, TCP `7881` as a WebRTC fallback, and UDP `7882` for encrypted media. If another local program already owns any of those ports, ChurchBoard automatically chooses the next free coordinated set and reports the active ports in the Producer Intercom module. The browser-facing signaling proxy still uses ChurchBoard's normal dashboard or Producer-workspace address. Remote use across the public internet requires professional TLS, TURN, firewall, and network design; automatic hosted mode is designed first for devices on the same church production network.
